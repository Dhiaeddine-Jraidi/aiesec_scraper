...
def third_stage(opp_id_sent_path, data_path, job, palm_api_list):
    ...
    filtered_df = df[(df['probability'] >= 20) & (~df['opportunity_id'].isin(opp_send_df['opportunity_id']))]
    ...
    for index, row in filtered_df.iterrows():
        ...
        country = row['country']
        soup = BeautifulSoup(role, 'html.parser')
        list_items = soup.find_all('li')
        formatted_message = '\n'.join(['• ' + item.get_text() for item in list_items])
